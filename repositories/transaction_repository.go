...
func (r *transactionRepository) GetSalesReportByDateRange(startDate, endDate time.Time) (model.SalesSummary, error) {
	var summary model.SalesSummary
	...
	err := r.db.QueryRow(context.Background(),
		`SELECT COALESCE(SUM(total_amount), 0) as total_revenue, COUNT(*) as total_transactions 
		 FROM transactions 
		 WHERE created_at >= $1 AND created_at < $2`,
		startDate, endDate).Scan(&summary.RevenueTotal, &summary.TransactionTotal)
	if err != nil {
		return summary, err
	}
	...
	err = r.db.QueryRow(context.Background(),
		`SELECT td.product_name, SUM(td.quantity) as total_qty
		 ...
		 ORDER BY total_qty DESC
		 LIMIT 1`,
		startDate, endDate).Scan(&bestProduct.Name, &bestProduct.Qty)
